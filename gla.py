...
import json
import copy
from pymongo import collection, DESCENDING
...
class GLA:
    def __init__(self, config_file: str, commit_collection: collection.Collection):
        ...
        self.default_folders = ['Source/media/media_driver/', 'Source/media/media_embargo/']
    ...
    def analyze_group_total_code_churn(self, date_range: list, groups: list) -> dict:
        """return group code churn analysis in date_range, classfied by category_list from give groups
           e.g Multiple groups's loc,files,commit times and regression times from 2018-10-01 to 2018-11-01
        Args:
        ...
        Returns:
            dict: {group1:{loc,files,commit,regression}, group1:{loc,files,commit,regression}, ...}
        """
    ...
    def analyze_group_code_churn_by_period(self, period: str, date_range: list, groups: list) -> dict:
        """return code churn analysis by period, in date_range, classified by category_list
           e.g Multiple groups's weekly loc,files,commit times and regression times from 2018-42 to 2018-48
        ...
        Returns:
            dict:  {'group1':{date:{loc,files,commit,regression},...},
                    'group2':{date:{loc,files,commit,regression},...},...}
        """
        d = {}
    ...
    def analyze_total_code_churn(self, date_range: list, category_list: list, classify_func, tmp_folders: list) -> dict:
        """return code churn analysis in date_range, classified by category_list
           e.g folder(s)'s loc,files,commit times and regression times,
           categorized by Platform from 2018-10-01 to 2018-11-01

        Args:
            date_range (list): [dayStart, dayEnd] yyyy-MM-DD inclusive
            category_list (list): all the components in the category
            classify_func ([type]): the function to classify each component
            tmp_folders (list): temporary folders to focus in this query only
        Returns:
            dict: {component1:{loc,files,commit,regression}, component2:{loc,files,commit,regression}, ...}
        """
        after, before = date_range
        total_file_set = FileSet()  # record all files latest name
    ...
    def analyze_code_churn_by_period(self, period: str, date_range: list, category_list: list, classify_func,
                                     tmp_folders: list) -> dict:
        """return code churn analysis by period, in date_range, classified by category_list
           e.g folder(s)'s weekly loc,files,commit times and regression times,
           categorized by Platform from 2018-42 to 2018-48
